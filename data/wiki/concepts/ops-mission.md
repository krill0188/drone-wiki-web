---
title: Drone Operations & Mission Overview
created: 2026-08-06
updated: 2026-08-06
type: concept
tags: [drone]
sources: []
confidence: medium
contested: false
contradictions: []
domain: ops-mission
---

# Drone Operations & Mission Overview

드론 운용/미션 도메인 개요 페이지. 공공안전(DFR), 배송, 검사, 재난 대응 등 실제 운용 응용 분야를 아우른다.

## 주요 응용 분야

| 분야 | 설명 |
|---|---|
| Drone as First Responder(DFR) | 응급 출동 전 드론 선행 대응 |
| 배송(Delivery) | 라스트마일 화물/의료품 배송 |
| 검사(Inspection) | 인프라·풍력터빈·통신탑 점검 |
| 재난 대응 | 항공 피해 평가, 실종자 수색 |

## 관련 개념

- [[drone-first-responder-dfr]] — 공공안전 DFR 프로그램
- [[drone-safety-failsafe]] — 운용 중 안전/failsafe 체계

## 운용 사례

- [[brinc-emergency-drone-funding]] — BRINC 응급 드론 펀딩(Motorola)
- [[cleveland-clinic-drone-delivery]] — Cleveland Clinic 장기 의료 드론 배달
- [[dronuum-computing-continuum]] — 드론 엣지 컴퓨팅 연속체
- [[high-speed-drone-tracking]] — 고속 드론 추적
- [[montis-avalanche-faa-approval]] — Montis 눈사태 구조 드론 FAA 승인
- [[perceptual-robotics]] — 풍력 터빈 검사 드론 플랫폼 기업
- [[skydio-centralsquare-dfr-integration]] — Skydio-CentralSquare DFR CAD 통합
- [[amazon-mk30-safety-incident]] — Amazon MK30 배송 드론 안전 사고

## 📰 최근 관련 소식
- BRINC Brings Wildfire and Smoke Tracking Into Drone Operations (dronelife.com, Mon, 10 Au) — https://dronelife.com/2026/08/10/brinc-wildfire-smoke-tracking-drone-operations/
