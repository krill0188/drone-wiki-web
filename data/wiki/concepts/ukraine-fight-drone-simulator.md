---
title: Ukrainian Fight Drone Simulator
created: 2026-08-14
updated: 2026-08-14
type: concept
tags: [drone, fpv, simulation]
sources: [inbox/fetch-2026-08-14-yt-ukraine-fight-drone-simulator-not-just-a-game.md]
confidence: medium
contested: false
contradictions: []
domain: drone-sw
---

# Ukrainian Fight Drone Simulator

UFDS(Ukrainian Fight Drone Simulator)는 일반적인 FPV 시뮬레이터가 아닌, 우크라이나 전투 드론 운용을 기반으로 한 특수 목적 시뮬레이터이다. Joshua Bardwell 채널에서 리뷰한 콘텐츠로, Steam에서 이용 가능하다.

## 개요

UFDS는 단순한 게임이 아닌 실제 전투 드론 운용 환경을 모사하는 시뮬레이터로, FPV 드론 파일럿 훈련 및 전술 연습에 활용될 수 있다.

## 플랫폼 정보

- **플랫폼**: Steam
- **앱 ID**: 2862860
- **장르**: FPV 드론 시뮬레이터

## 관련 페이지

- [[drone-simulation]] — Gazebo, jMAVSim, SITL, 멀티 기체 시뮬레이션
- [[fpv-hardware]] — 2026년 7월 FPV 드론 하드웨어 동향
- [[betaflight]] — FPV 드론용 오픈소스 비행 제어 소프트웨어

## 📰 최근 관련 소식
- [Joshua Bardwell] Ukraine Fight Drone Simulator // NOT JUST A GAME (youtube.com, 2026-08-13) — https://www.youtube.com/watch?v=_3blDd5NZRM
