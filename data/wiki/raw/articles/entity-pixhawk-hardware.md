---
title: "Pixhawk Flight Controller — Entity Reference"
created: 2026-07-28
captured: 2026-07-28
type: article
url: "https://docs.px4.io/main/en/flight_controller/"
author: "Pixhawk / PX4 Docs / Master"
sha256: ""
tags: [drone-hw, drone-sw]
---

# Pixhawk Flight Controller — Entity Reference

## 개요

Pixhawk는 PX4 프로젝트와 함께 개발된 오픈 하드웨어 비행 제어기(FC) 플랫폼이다.
Holybro, mRo, CubePilot 등 다수 제조사가 공식 Pixhawk 표준 호환 제품을 생산한다.
...
*Pixhawk 6X는 IMU 3개, 자력계 2개 내장 (이중화)*

## 커넥터 표준 (Pixhawk Connector Standard)
...
```bash
# QGroundControl로 펌웨어 업로드
# Vehicle Setup → Firmware → PX4 Pro Stable

# CLI로 직접 빌드·업로드
make holybro_pixhawk6c_default upload

# NuttX 셸 접속 (MAVLink Console)
...
```bash
# DroneCAN ESC 탐색 (NuttX 셸)
uavcan start
uavcan status

# DroneCAN 파라미터
UAVCAN_ENABLE = 3       # DroneCAN + ESC 출력 활성화
UAVCAN_NODE_ID = 1      # 비행 제어기 노드 ID
```

## 관련 엔티티

- [[PX4]] — Pixhawk 주요 구동 비행 스택
- [[ArduPilot]] — Pixhawk에서 실행 가능한 대안 스택
- [[DroneCAN]] — Pixhawk CAN 버스 프로토콜
- [[MAVLink]] — 지상국 통신 프로토콜
