---
title: "PX4 Flight Stack — Entity Reference"
created: 2026-07-28
captured: 2026-07-28
type: article
url: "https://docs.px4.io/main/en/"
author: "PX4 Dev Team / Master"
sha256: ""
tags: [drone-sw]
---

# PX4 Flight Stack — Entity Reference
...
| `ekf2` | 확장 칼만 필터 기반 상태 추정 (위치·자세·속도) | `vehicle_local_position`, `vehicle_attitude` |
| `sensors` | 센서 데이터 수집·캘리브레이션·퓨전 전처리 | `sensor_combined`, `sensor_gyro` |
| `mavlink` | MAVLink 프로토콜 통신 (GCS·온보드 컴퓨터) | `mavlink_log`, `telemetry_status` |
...
| `NAV_RCL_ACT` | 2 | RC 신호 손실 시 동작 (0=없음, 1=착륙, 2=RTL) |
| `COM_DISARM_LAND` | 2.0 s | 착륙 후 자동 Disarm 시간 |
| `SYS_AUTOSTART` | 4001 | 기체 프레임 자동 설정 (4001=쿼드콥터X) |
...
| Offboard | OFFBOARD | 외부 컴퓨터 제어 (MAVSDK/ROS2) |
...
| `actuator_controls_0` | mc_att_control | mixer | 제어 출력 (roll/pitch/yaw/thrust) |
...
# MAVSDK Python으로 연결
pip install mavsdk
python3 scripts/offboard_control.py
```

## 관련 엔티티

- [[ArduPilot]] — 대표적인 경쟁/대안 비행 스택
- [[MAVLink]] — 지상국-비행체 통신 프로토콜
- [[Pixhawk]] — PX4 주요 구동 하드웨어 플랫폼
- [[ROS2-PX4-Bridge]] — ROS2 연동 인터페이스 (uXRCE-DDS)
